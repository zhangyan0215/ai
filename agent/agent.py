...
class ReActAgent:
    ...
    def render_system_prompt(self, system_prompt_template: str) -> str:
        ...
        # 添加工作目录说明
        working_dir_instruction = f"\n\n重要提示：当前工作目录是 '{self.project_directory}'。所有文件操作都应该在此目录中。例如，要写入当前目录下的 'snake.html' 文件，请使用 '{self.project_directory}/snake.html' 需要是完整路径。"

        return (
            Template(system_prompt_template).substitute(
                operating_system=self.get_operating_system_name(),
                tool_list=tool_list,
                file_list=file_list,
            )
            + working_dir_instruction
        )
    ...
    def call_model(self, messages):
        print("\n\n正在请求模型，请稍等...")
        print("messages is: {messages}".format(messages=messages))
...
def main(project_directory):
    ...
    agent = ReActAgent(tools=tools, model="glm-4.7", project_directory=project_dir)
